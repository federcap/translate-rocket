...
* Fix: every admin screen fits a phone. The translation panel under the page editor squeezed the translation to one letter per line; it now shows one card per sentence, original above and translation below. The same for the translation memory, the list of pages and the progress per language; the AI provider cards no longer run past the right edge, and the language visibility menus fit the editor sidebar.
* Fix: the floating language switcher no longer covers the buttons of a cookie banner. It stays above banners, as since 1.5.2, but now also moves up above any bar fixed to the bottom of the screen, and comes back down when the bar goes away.
* Fix: the setup wizard shows only its three steps, without notices from TranslateRocket or other plugins.
* Improved: in the list of pages, «Redo» is now «Start over» and «✖» is now «Hide» (nothing is deleted), with an info icon explaining every button; the Undo link says what it puts back.
* Tested on a phone-sized and a desktop screen, 17 screens and pages checked automatically, including the visual editor and the public site as a visitor.
...
* Fix: from TranslatePress, a sentence left identical by machine translation is no longer imported as its own translation. A name you kept identical on purpose, approved by a person, still is - so automatic translation never touches it afterwards.
* Fix: a TMX file exported from TranslateRocket and imported again no longer loses the translation of labels and social titles that share their words with page text. A TMX from another tool now works with straight apostrophes and dashes, and reaches every place those words are used on the site.
* Tested with the real plugins, in Italian, German, French, Arabic and Chinese: Bogo, TranslatePress, qTranslate-XT, WPGlobus, WP Multilang, Multilanguage, Loco Translate, plus Weglot and TMX/CSV files.

= 1.5.0 =
* New: a sentence broken by a link or a bold word is now translated whole. Until now "Read the <a>booking conditions</a> before you confirm." reached the translator as three separate pieces, so nobody could put the words in the order German or Japanese needs, and "Read the" on its own is not translatable at all. The sentence now travels in one piece with numbered marks where the tags are, and the translation is put back into your own links: the address of a link always comes from your page, never from the translation. Sentences already translated keep what they have, so nothing you have done is lost, but the count of phrases to translate will go up: the whole sentences are new to the plugin.
* New: "Next steps", a screen of its own, first in the TranslateRocket menu, with a number beside it when something is waiting. It says where your site is right now and what is left to do, with a button for each step, and it is worked out from your site every time you open it. The same panel appears at the top of the main screen. It also says, plainly, when there is nothing to do: the steps that tell you to leave something alone matter as much as the ones that ask for work.
* New: while another translation plugin is active, an administrator looking at the public site now sees one line saying that TranslateRocket is standing aside on purpose and nothing is broken. Somebody spent hours hunting a fault that was not there, because the right behaviour and a broken plugin looked identical.
* New: the menus another plugin keeps one per language are now named on the Import screen, with the reason you can leave them exactly where they are. Switching that plugin off keeps the menu of your main language in every position; the others simply stop being used.
* New: deactivating TranslateRocket asks once, and only once, what went wrong. Answering is optional, the plugin deactivates either way, and nothing is sent anywhere: the answer stays on your site and a button copies it for you if you want to send it yourself.
* New: the support screen and the support page now say exactly what makes a report easy to fix: the address of a page, the exact words, and the piece of the page copied as code.
* Fix: a page built with a page builder now brings its translations with it. Elementor keeps every word of a page in a meta field of its own, not in the page content, so importing from WPML or Polylang used to bring over the title and the excerpt and nothing else — on exactly the sites with the most translation work behind them, since Polylang or WPML plus Elementor is one of the commonest ways to build a multilingual site. The two copies are now read side by side and paired widget by widget, by the id of the widget, so a widget added in one language only does not throw everything after it out of line. Anything identical in both languages is left alone, and an address is never stored as the translation of another address. Bricks is read the same way. Found while looking into what a user who left had been through.
* Fix: translations imported from Polylang, WPML and the other plugins that keep a copy of each page per language are now actually used when a sentence has an apostrophe, quotes, a dash or an ampersand. WordPress shows "don't" as "don’t" and " - " as " – ": the imported sentence was stored as it was typed, the page was read as it is shown, the two never matched, and the page stayed in the source language with nothing to say why. Both shapes are stored now.
* Fix: a sentence that is the same in both copies is no longer imported as its own "translation". It used to count as translated, stay in the source language on the translated page, and be skipped by automatic translation for good.
* Fix: an ampersand in an imported translation no longer shows up as "&amp;" on the translated page.
* Fix: blocks that keep their text in their settings rather than in their HTML — Spectra 3 is one — are imported too. On a page built that way, only a handful of lines used to come across.
* Fix: an imported paragraph with a link or a bold word in it is now used on the translated page. Since 1.5.0 the engine reads such a sentence whole, with numbered marks where its links are; the importer stored it with the links stripped, so the two never matched. Imported sentences now take the engine's own shape, and are paired only when both copies carry the same links in the same places.
* Fix: the import no longer takes styles (gradients, colours), a pattern's own description or text damaged by a bad save for sentences to translate.
* Fix: after the switch from WPML or Polylang, an old address under that plugin's own language prefix - /zh-hans/ where TranslateRocket uses /zh/, and likewise zh-hant or pt-pt - answered "page not found". It now leads to the translated page with a permanent redirect. Only prefixes the other plugin really used on your site are recognised, only addresses that would otherwise be "not found" are touched, and nothing changes while the other plugin is still active.
* Fix: a copy left by WPML or Polylang whose text had all been imported was reported as having one text not imported when a sentence contained an ampersand or a typographic apostrophe, so it was offered to be kept and stayed online as a second page. The review now compares the sentences in the same shapes the import stores.
* Fix: the advice about headers and footers in several languages never actually appeared. The templates were read with a query that Polylang and WPML filter by language, so the plugin saw one language, concluded there was nothing to say, and stayed silent — exactly on the sites that needed the advice. They are now read straight from the database. The same screen also names the menus another plugin keeps per language, and the product template of a page builder, which were missed before.
* Fix: the advice about headers and footers in several languages used to say the copies "can go" without saying that you do not have to change anything and that they come straight back from the Trash. Written that way, it could cost somebody their header.
* Fix: WordPress strips anything between angle brackets when it cleans up a form field, which would have thrown away the marks inside a sentence: a correction typed by hand could never have been saved. Corrections that lose those marks are now refused with an explanation instead of being stored unusable.
...
Versions 0.1.0 to 0.10.0 are listed in changelog.txt, shipped with the
